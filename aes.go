...
var sboxes [256]uint8 = [256]uint8{
...
var invSboxes [256]uint8 = [256]uint8{
...
func getSbox(index uint8) uint8 {
	return sboxes[index]
}

func getInvSbox(index uint8) uint8 {
	return invSboxes[index]
}
...
func aesMixColumns(block aesBlock) {
	for i := 0; i < 4; i++ {
		...
		tm := block[i][0] ^ block[i][1]
		tm = aesXtime(tm)
		block[i][0] ^= tm ^ tmp

		tm = block[i][1] ^ block[i][2]
		tm = aesXtime(tm)
		block[i][1] ^= tm ^ tmp

		tm = block[i][2] ^ block[i][3]
		tm = aesXtime(tm)
		block[i][2] ^= tm ^ tmp
